  ...
  return (
    <div className='item' onClick={() => setState(prev => !prev)}>
      <p className={`ttl ${state ? 'active' : ''}`}>
        <img src={state ? minusIcon : plusIcon} style={{marginRight: '20px'}} />{title || 'Who is Metoshi?'}</p>
      <div className="item_in" style={{ display: state ? 'block' : 'none' }} dangerouslySetInnerHTML={{ __html: text }} />
    </div>
  )
}
